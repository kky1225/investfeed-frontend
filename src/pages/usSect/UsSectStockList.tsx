import Typography from "@mui/material/Typography";
import Box from "@mui/material/Box";
import Grid from "@mui/material/Grid";
import {GridColDef} from "@mui/x-data-grid";
import {useEffect, useMemo, useRef, useState} from "react";
import {useParams} from "react-router-dom";
import {UsSectStockGridRow, UsSectStockListItem, UsSectStockListReq, UsSectStockListRes,} from "../../type/UsSectType.ts";
import {fetchUsSectStockList, fetchUsSectStockStream} from "../../api/usSect/UsSectApi.ts";
import UsSectStockTableProps from "../../components/UsSectStockTable.tsx";
import {renderChip} from "../../components/CustomRender.tsx";
import FreshnessIndicator from "../../components/FreshnessIndicator.tsx";
import {usePollingQuery} from "../../lib/pollingQuery.ts";
import {EXCHANGE_LABEL} from "../../lib/exchange.ts";
import {StockStream, StockStreamRes} from "../../type/StockType.ts";

interface LiveUsSectStockUpdate {
    curPrc: string;
    fluRt: string;
    trend: string;
}

const DEFAULT_REQ: UsSectStockListReq = {sortTp: "1"};

const UsSectStockList = () => {
    const {indsCd} = useParams();
    const sectIndsCd = indsCd || "000";

    const [liveOverlay, setLiveOverlay] = useState<Map<string, LiveUsSectStockUpdate>>(new Map());
    const stockBufferMap = useRef<Map<string, StockStream>>(new Map());
    const subscribedKeyRef = useRef<string>('');

    const {data: result, isLoading, lastUpdated, pollError} = usePollingQuery<UsSectStockListRes>(
        ['usSectStockList', sectIndsCd, DEFAULT_REQ.sortTp],
        (config) => fetchUsSectStockList(sectIndsCd, DEFAULT_REQ, config),
    );

    // 키움 응답에 시각 필드가 없어 stamp 비교 불가. 폴링 도착 시 WS overlay 를
    // 비워서 stale 가격이 새 폴링 결과를 덮어쓰지 못하게 한다.
    useEffect(() => {
        if (!result) return;
        setLiveOverlay(new Map());
    }, [result]);

    const row: UsSectStockGridRow[] = useMemo(() => {
        if (!result) return [];
        return (result.sectStockList ?? []).map((sectStock: UsSectStockListItem) => {
            const live = liveOverlay.get(sectStock.stkCd);
            return {
                id: sectStock.stkCd,
                stexTp: sectStock.stexTp,
                stkNm: sectStock.stkNm,
                fluRt: live?.fluRt ?? sectStock.fluRt,
                curPrc: live?.curPrc ?? sectStock.curPrc,
                accTrdeQty: sectStock.accTrdeQty,
            };
        });
    }, [result, liveOverlay]);

    const loading = isLoading;

    const openSocket = () => {
        const socket = new WebSocket("ws://localhost:8080/ws");
        socket.onmessage = (event) => {
            const data = JSON.parse(event.data);
            if (data.trnm === "REAL" && Array.isArray(data.data)) {
                data.data.forEach((entry: StockStreamRes) => {
                    const values = entry.values;
                    if (values?.["10"] == null) return;
                    stockBufferMap.current.set(entry.item, {
                        code: entry.item,
                        value: String(values["10"]).replace(/^[+-]/, ''),
                        fluRt: String(values["12"]),
                        predPre: String(values["11"]),
                        trend: String(values["25"]),
                    });
                });
            }
        };
        return socket;
    };

    // WebSocket 라이프사이클 — query 결과 종목들로 stream 등록.
    // 미국 장 시간 게이팅은 백엔드(isUsOpen)에서 처리하므로 여기서는 바로 등록한다.
    useEffect(() => {
        if (!result) return;
        const items = (result.sectStockList ?? []).map((s: UsSectStockListItem) => ({stkCd: s.stkCd, stexTp: s.stexTp}));
        if (items.length === 0) return;

        const key = `${sectIndsCd}|${items.map((i) => i.stkCd).join(',')}`;
        if (subscribedKeyRef.current === key) return;
        subscribedKeyRef.current = key;

        // indsCd 변경 시 overlay reset
        setLiveOverlay(new Map());

        let displayInterval: ReturnType<typeof setInterval>;
        let socket: WebSocket | undefined;

        const startDisplayLoop = () => {
            displayInterval = setInterval(() => {
                if (stockBufferMap.current.size === 0) return;
                setLiveOverlay((prev) => {
                    const next = new Map(prev);
                    stockBufferMap.current.forEach((v, k) => {
                        next.set(k, {
                            curPrc: v.value,
                            fluRt: v.fluRt,
                            trend: v.trend,
                        });
                    });
                    return next;
                });
                stockBufferMap.current.clear();
            }, 200);
        };

        (async () => {
            await fetchUsSectStockStream({items});
            socket = openSocket();
            startDisplayLoop();
        })();

        return () => {
            socket?.close();
            clearInterval(displayInterval);
        };
    }, [result, sectIndsCd]);

    const columns: GridColDef[] = [
        {
            field: 'index',
            headerName: '번호',
            flex: 1,
            minWidth: 80,
            maxWidth: 80,
            renderCell: (params) => {
                return params.api.getAllRowIds().indexOf(params.id) + 1;
            }
        },
        {
            field: 'stkNm',
            headerName: '주식 이름',
            flex: 1.5,
            minWidth: 180
        },
        {
            field: 'stexTp',
            headerName: '거래소',
            flex: 0.5,
            minWidth: 90,
            valueFormatter: (param: string) => EXCHANGE_LABEL[param] ?? param,
        },
        {
            field: 'fluRt',
            headerName: '등락률',
            flex: 0.5,
            minWidth: 100,
            renderCell: (params) => renderChip(params.value as number),
        },
        {
            field: 'curPrc',
            headerName: '현재가',
            flex: 1,
            minWidth: 110,
            valueFormatter: (param: string) => `$${Number(String(param).replace(/^[+-]/, '')).toLocaleString()}`,
        },
        {
            field: 'accTrdeQty',
            headerName: '거래량',
            flex: 1,
            minWidth: 100,
            valueFormatter: (param: string) => Number(String(param).replace(/^[+-]/, '')).toLocaleString(),
        }
    ];

    return (
        <Box sx={{width: '100%', maxWidth: {sm: '100%', md: '1700px'}}}>
            <Box sx={{display: 'flex', alignItems: 'center', mb: 2, gap: 2}}>
                <Typography component="h2" variant="h6">
                    미국 업종 주식 목록
                </Typography>
                <Box sx={{flex: 1}}/>
                <FreshnessIndicator lastUpdated={lastUpdated} error={pollError}/>
            </Box>
            <Grid
                container
                spacing={2}
                columns={12}
                sx={{mb: (theme) => theme.spacing(2)}}
            >
                <Box sx={{width: '100%'}}>
                    <UsSectStockTableProps rows={row} columns={columns} pageSize={100} loading={loading}/>
                </Box>
            </Grid>
        </Box>
    );
};

export default UsSectStockList;
