...
export default function CryptoManualHoldingTab({broker}: CryptoManualHoldingTabProps) {
    ...
    const loadData = useCallback(async () => {
        try {
            const data = await fetchCryptoManualHoldingList(broker.id);
            ...
            setMarkets(items.map(i => i.stkCd));
            setOrderDirty(false);
        } catch (err) {
            console.error(err);
        } finally {
    ...
    const handleStreamUpdate = useCallback((bufferMap: Map<string, CryptoHoldingBuffer>) => {
        setHoldings(prev => {
            ...
            bufferMap.forEach((buffer, market) => {
                if (buffer.curPrc) priceMap.set(market, buffer.curPrc);
            });
    ...
    const handleDelete = async () => {
        ...
        try {
            await deleteCryptoManualHolding(deleteTarget.id);
            await loadData();
        } catch (err) {
            console.error(err);
        }
    ...
    const handleSaveOrder = async () => {
        setSavingOrder(true);
        try {
            await reorderCryptoHoldings({orderedIds: holdings.map(h => h.id)});
            setOrderDirty(false);
        } finally {
    ...
    const handleBalanceUpdate = async (newBalance: number) => {
        try {
            await updateCryptoBrokerBalance(broker.id, {balance: newBalance});
