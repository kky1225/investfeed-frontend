...
export default function ManualHoldingTab({broker}: ManualHoldingTabProps) {
    ...
    const {data: holdingData, isLoading: loading} = useQuery<ManualHoldingData>({
        ...
        queryFn: async ({signal}) => requireOk<ManualHoldingData>(await fetchManualHoldingList(broker.id, {signal, skipGlobalError: true}), null),
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id: number) => {
            requireOk(await deleteManualHolding(id), '수동 보유주식 삭제');
        },
        onSuccess: () => {
            loadData();
    ...
    const reorderMutation = useMutation({
        mutationFn: async (req: HoldingReorderReq) => {
            requireOk(await reorderHoldings(req), '보유주식 순서 변경');
        },
        onSuccess: () => {
            setOrderDirty(false);
            queryClient.invalidateQueries({queryKey: ['manualHoldingList', broker.id]});
        },
    ...
    const balanceMutation = useMutation({
        mutationFn: async (req: UpdateBalanceReq) => {
            requireOk(await updateBrokerBalance(broker.id, req), '계좌 잔액 수정');
            return req.balance;
        },
