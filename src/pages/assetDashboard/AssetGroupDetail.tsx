...
export default function AssetGroupDetail({group, summary}: AssetGroupDetailProps) {
    ...
    useHoldingStream(stockCodes, handleStockStreamUpdate);
    useCryptoHoldingStream(cryptoCodes, handleCryptoStreamUpdate);
    ...
    const columns: GridColDef[] = [
        ...
        {field: 'evltAmt', headerName: '평가금액', flex: 1, minWidth: 120, valueFormatter: (value: number) => value.toLocaleString()},
        {field: 'evltPl', headerName: '평가손익', flex: 1, minWidth: 120, renderCell: (params: {value: number}) => renderTradeColor(params.value)},
        {field: 'purAmt', headerName: '매입금액', flex: 1, minWidth: 120, valueFormatter: (value: number) => value.toLocaleString()},
                    ...
                    <Stack direction="row" spacing={4} divider={<Divider orientation="vertical" flexItem/>} sx={{mb: 3}}>
                        <Box>
                            ...
                            <Typography variant="body1" sx={{fontWeight: 600}}>
                                {groupSummary.evltAmt.toLocaleString()}원
                            </Typography>
                        </Box>
                        <Box>
                            <Typography variant="body2" sx={{color: 'text.secondary'}}>투자원금</Typography>
                            <Typography variant="body1" sx={{fontWeight: 600}}>
                                {groupSummary.purAmt.toLocaleString()}원
                            </Typography>
                        </Box>
                        <Box>
                            ...
                            <Typography variant="body1" sx={{fontWeight: 600, color: profitColor}}>
                                {groupSummary.evltPl > 0 ? '+' : ''}{groupSummary.evltPl.toLocaleString()}원 ({Number(groupSummary.prftRt) > 0 ? '+' : ''}{groupSummary.prftRt}%)
                            </Typography>
                        </Box>
                        <Box>
                            <Typography variant="body2" sx={{color: 'text.secondary'}}>예수금</Typography>
                            <Typography variant="body1" sx={{fontWeight: 600}}>
                                {groupSummary.cash.toLocaleString()}원
                            </Typography>
                        </Box>
                    </Stack>
